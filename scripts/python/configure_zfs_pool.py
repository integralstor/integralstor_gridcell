...
def configure_zfs_pool():
  ...
  try :
    ...
    str_to_print = 'Select a ZFS pool type (1-%d) (Default is %s, Press enter to select to default) : '%(len(pool_types), default_str)
    valid_input = False
    pool_num = -1
    ...
    if selected_pool_type in ['raid5', 'raid6']:
      vdev_list, err = zfs.create_pool_data_vdev_list(selected_pool_type, num_raid_disks = num_free_disks)
    elif cd['selected_pool_type'] == 'raid10':
      vdev_list, err = zfs.create_pool_data_vdev_list(selected_pool_type, stripe_width = num_free_disks/2)
    else:
    ...
    result, err = zfs.create_pool('frzpool', selected_pool_type, vdev_list, dedup = True)
    if not result:
      if not err:
    ...
    if err:
      raise Exception(err)
    ret, err = zfs.create_dataset('frzpool', 'deduplicated', {'dedup':'on'})
    #print 'create dedup', ret, err
    if err:
      raise Exception(err)
    ret, err = zfs.create_dataset('frzpool', 'compressed', {'compression':'on'})
