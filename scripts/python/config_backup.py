...
def zip_gridcell_gluster_config():
  ...
  try:
    ...
    zf = zipfile.ZipFile('/tmp/gluster_config_%s.zip'%hn, 'w', zipfile.ZIP_DEFLATED)
    ...
    shutil.move('/tmp/gluster_config_%s.zip'%hn, '%s/config_backup/gridcells/%s.zip'%(config_dir, hn))
  except Exception, e:
...
def zip_grid_gluster_config():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    zf = zipfile.ZipFile('/tmp/grid_gluster_config.zip', 'w', zipfile.ZIP_DEFLATED)
    ...
    shutil.move('/tmp/grid_gluster_config.zip', '%s/config_backup/grid/gluster_config.zip'%log_dir)
  except Exception, e:
...
def main():
  ...
  try:
    lg, err = logger.get_script_logger('Config backup', '/var/log/integralstor/scripts.log', level = logging.DEBUG)

    if len(sys.argv) != 2 or sys.argv[1].strip() not in ['backup_gridcell_config','backup_grid_config']:
      raise Exception('Usage: python config_backup.py [backup_gridcell_config|backup_grid_config]')
