...
def check_salt_status(print_to_stdout = True, separator = '\n'):
  ...
  try:
    if print_to_stdout:
      ret_list.append('IntegralStor Monitoring agents and processes status')

    out, err = command.get_command_output('service salt-master status', False)
    if err:
      raise Exception(err)
    ret_list.append('Monitoring service master status : %s'%','.join(out))

    out, err = command.get_command_output('service salt-minion status', False)
    if err:
      raise Exception(err)
    ...
    if err:
      raise Exception(err)
    if keys:
      ret_list.append(separator+'Admin agents that are part of the grid : ')
      ret_list.append(separator.join(keys['minions']))
      ret_list.append(separator+'Admin agents that are pending to be accepted into grid : ')
...
def check_admin_vol_status(print_to_stdout = True, separator = '\n'):
  ...
  try:
    ...
    out, err = command.get_command_output('gluster volume status integralstor_admin_vol ')
    if err:
      raise Exception(err)
    ret_list.extend(out)
    if print_to_stdout:
  ...
  try:
    ...
    if all_disks:
      for serial_num, info in all_disks.items():
        ret_list.append('Serial number : %s, capacity : %s, Status : %s'%(serial_num, info['capacity'], info['status']))
    if print_to_stdout:
      print separator.join(ret_list)
  except Exception, e:
    return None, 'Error checking hardware status: %s'%str(e)
...
def check_windows_processes_status(print_to_stdout = True, separator = '\n'):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    ret_list.append('Windows (winbind) process status : %s'%', '.join(out))
...
def check_integralview_processes_status(print_to_stdout = True, separator = '\n'):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    ret_list.append('Web application server (uwsgi) status : %s'%', '.join(out))
    if print_to_stdout:
      print separator.join(ret_list)
  except Exception, e:
...
def check_gluster_processes_status(print_to_stdout = True, separator = '\n'):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    ret_list.append('Distributed storage service (glusterd)  daemon status : %s'%', '.join(out))
    out, err = command.get_command_output('service glusterfsd status', False)
    if err:
      raise Exception(err)
    ret_list.append('Distributed storage file service (glusterfsd)  status : %s'%', '.join(out))
    if print_to_stdout:
      print separator.join(ret_list)
  except Exception, e:
    return None, 'Error checking windows access process status: %s'%str(e)
...
def generate_status_file():
  try:
    ...
    #with open('/opt/integralstor/integralstor_gridcell/integral_view/static/test.html', 'w') as f:
      f.write( '<html>\n')
      f.write( '<head>\n')
      f.write( '<title>IntegralSTOR GRIDCell status</title>\n')
      f.write(' <link rel="icon" href="/static/images/favicon.ico">\n')
      f.write( '<link rel="stylesheet" href="/static/bootstrap/css/bootstrap.css" type="text/css" />\n')
      f.write( '<link rel="stylesheet" href="/static/bootstrap/css/bootstrap-responsive.min.css"/>\n')
      f.write( '<link rel="stylesheet" type="text/css" href="/static/css/AdminLTE.css">\n')
      f.write( '<link rel="stylesheet" type="text/css" href="/static/css/skin-purple.css">\n')
      f.write( '<link rel="stylesheet" type="text/css" href="/static/css/fractalio.css">\n')
      f.write( '</head>\n')
