...
def configure_addresses():
  try:
    ...
    while True:
      ...
      action_choice = ''
      while True:
      #while action_choice = raw_input('Enter number corresponding to the desired action: '):
        action_choice = raw_input('Enter number corresponding to the desired action: ')
      ...
      while True:
        if action_choice == '1':
          ...
          if confirm.lower() != 'y':
            ...
            ret, err = ctdb.mod_public_addresses(ip_list, action='add')
            if err:
          ...
          if confirm.lower() != 'y':
            ...
            ret, err = ctdb.mod_public_addresses(ip_list, action='remove')
            if err:
