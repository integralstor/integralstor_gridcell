...
def process_batch(d, file, logger=None):
    ...
    try:
            ...
            if cd["type"] == "volume_heal_full":
                ...
                try:
                    ...
                    if l:
                        ...
                        # No heal count returned so dont know what happened -
                        # flag error
                        raise Exception("Could not get heal count")
                except Exception, e:
                    cd["status_code"] = -1
                    cd["err_msg"] = "Error executing volume heal info command : %s" % str(
                        e)
                    # Stop executing more commands!
                    break
                ...
                try:
                    ...
                    if err:
                        raise Exception(
                            "Error parsing xml output from %s : %s" % (cd["type"]), err)
                    ...
                    if cd["type"] in ["remove_brick_status", "rebalance_status"]:
                        ...
                        if nodes:
                            for node in nodes:
                                status_str, err = xml_parse.get_subnode_text(
                                    node, "status")
                                if err:
                                    raise Exception(err)
                        ...
                        try:
                            ret, err = xml_parse.get_subnode_text(
                                node, "files")
                            cd["files"] = int(ret)
                        ...
                        # If the nodes signal done then confirm that the
                        # aggregate also says so - Quirk with gluster xml
                        # output
                        if done:
                            if node:
                                try:
        ...
        # Write the updated json to a temp file and copy to prevent possible
        # race read/write conditions?
        ...
        with open(os.path.normpath("%s/tmp_%s" % (batch_files_path, file)), "w+") as f1:
            ...
            f1.flush()
            f1.close()
        shutil.move(os.path.normpath("%s/tmp_%s" % (batch_files_path, file)),
