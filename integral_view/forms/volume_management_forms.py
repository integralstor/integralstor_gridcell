...
class VolumeOptionsForm(forms.Form):
    ...
    def clean(self):
        ...
        if "enable_worm" in cd:
            if cd["enable_worm"]:
                if cd["readonly"] == "on":
                    self._errors["readonly"] = self.error_class(
                        ["You cannot enable Read only and the WORM feature together"])
        return cd
    ...
    def clean_snapshot_name(self):
        ...
        if not snapshot_name[0].isalpha():
            raise forms.ValidationError(
                "The snapshot name should begin with an alphabet")
        if (' ' in snapshot_name):
            raise forms.ValidationError(
                "The snapshot name cannot contain spaces")
...
class ReplaceNodeForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for hostname in src_node_list:
            ...
            ch.append(tup)
        self.fields["src_node"] = forms.ChoiceField(
            widget=forms.RadioSelect(), choices=ch)

        for hostname in dest_node_list:
            tup = (hostname, hostname)
            ch.append(tup)
        self.fields["dest_node"] = forms.ChoiceField(
            widget=forms.RadioSelect(), choices=ch)
    ...
    def clean(self):
        ...
        if src_node == dest_node:
            raise forms.ValidationError(
                "Source and replacement GRIDCells should be different")
        return cleaned_data
