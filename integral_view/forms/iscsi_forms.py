  ...
  '''
    ...
    def __init__(self, *args, **kwargs):
        ...
        for v in vl:
            tup = (v, v)
            ch.append(tup)
        self.fields["vol_name"] = forms.ChoiceField(
            widget=forms.Select, choices=ch)
        ch = []
        for a in aal:
            tup = (a["id"], a["id"])
            ch.append(tup)
        self.fields["auth_group_id"] = forms.ChoiceField(
            widget=forms.Select, choices=ch)
        ch = []
        for i in igl:
            tup = (i["id"], i["id"])
            ch.append(tup)
        self.fields["init_group_id"] = forms.ChoiceField(
            widget=forms.Select, choices=ch)
...
class AuthorizedAccessUserForm(forms.Form):
    ...
    auth_access_group_id = forms.IntegerField(
    ...
    def clean(self):
        ...
        if cd["secret"] != cd["secret_conf"]:
            self._errors["secret_conf"] = self.error_class(
                ["Secret and confirmation do not match"])
            del cd["secret_conf"]
        '''
...
class TargetGlobalConfigForm(forms.Form):
    ...
    nop_in_interval = forms.IntegerField(
        min_value=0, max_value=300, initial=20)
    max_sessions = forms.IntegerField(min_value=1, max_value=65536, initial=16)
    max_connections = forms.IntegerField(
        min_value=1, max_value=65536, initial=8)
    max_presend_r2t = forms.IntegerField(
        min_value=1, max_value=255, initial=32)
    max_outstanding_r2t = forms.IntegerField(
        min_value=1, max_value=255, initial=16)
    first_burst_length = forms.IntegerField(
        min_value=1, max_value=4294967296, initial=65536)
    max_burst_length = forms.IntegerField(
        min_value=1, max_value=4294967296, initial=262144)
    max_receive_data_segment_length = forms.IntegerField(
        min_value=1, max_value=4294967296, initial=262144)
    default_time_to_wait = forms.IntegerField(
        min_value=1, max_value=300, initial=2)
    default_time_to_retain = forms.IntegerField(
        min_value=1, max_value=300, initial=60)
    '''
  ...
  '''
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["discovery_auth_group"] = forms.ChoiceField(
            widget=forms.Select, choices=ch)
