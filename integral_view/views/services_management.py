    ...
    try:
        ...
        if "ack" in request.GET:
            ...
            if request.GET["ack"] == "start":
                return_dict['ack_message'] = 'Service "%s" started on GRIDCell %s. The status will reflect on this screen after one minute.' % (
                    sn, gridcell)
            elif request.GET["ack"] == "stop":
                return_dict['ack_message'] = 'Service "%s" stopped on GRIDCell %s. The status will reflect on this screen after one minute.' % (
                    sn, gridcell)
            if request.GET["ack"] == "restart":
                return_dict['ack_message'] = 'Service "%s" restarted on GRIDCell %s. The status will reflect on this screen after one minute.' % (
                    sn, gridcell)
