...
def edit_cifs_share(request):
  ...
  try:
    ...
    if request.method == "GET":
      ...
      form = cifs_shares_forms.ShareForm(initial = initial, user_list = user_list, group_list = group_list, volume_list = vil)
      ...
      form = cifs_shares_forms.ShareForm(request.POST, user_list = user_list, group_list = group_list, volume_list = vil)
      return_dict["form"] = form
      if form.is_valid():
...
def create_cifs_share(request):
  ...
  try:
    ...
    if request.method == "GET":
      #Return the form
      form = cifs_shares_forms.CreateShareForm(user_list = user_list, group_list = group_list, volume_list = vl)
      return_dict["form"] = form
      ...
      form = cifs_shares_forms.CreateShareForm(request.POST, user_list = user_list, group_list = group_list, volume_list = vl)
      return_dict["form"] = form
      #print request.POST
      if form.is_valid():
        #print return_dict
  ...
  try:
    ...
    if request.method == 'GET':
      ...
      if d and d["security"] == "ads":
        form = cifs_shares_forms.ADAuthenticationSettingsForm(initial=ini)
      else:
        form = cifs_shares_forms.LocalUsersAuthenticationSettingsForm(initial=ini)
      return_dict["form"] = form
      ...
      if request.POST["security"] == "ads":
        form = cifs_shares_forms.ADAuthenticationSettingsForm(request.POST)
      elif request.POST["security"] == "users":
        form = cifs_shares_forms.LocalUsersAuthenticationSettingsForm(request.POST)
      else:
        raise Exception("Invalid security specification. Please try again using the menus")
