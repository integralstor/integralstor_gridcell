...
def view_gridcells(request):
  ...
  try:
    ...
    for hostname in si.keys():
      ...
      if ret:
        rnl.append(hostname)
      ret, err = gluster_trusted_pools.can_add_gridcell_to_storage_pool(hostname, si)
      if err:
        raise Exception(err)
      if ret:
        anl.append(hostname)
...
def view_gridcell(request):
  ...
  try:
    ...
    #Figure out whether to callout errors for each component in the template!
...
def scan_for_new_gridcells(request):
  ...
  try:
    ...
    if request.method == 'GET':
      ...
      if pending_minions:
        form = gridcell_management_forms.AddGridcellsForm(pending_minions_list = pending_minions)
        return_dict["form"] = form
      else:
        return_dict["no_new_gridcells"] = True
    else:
      form = gridcell_management_forms.AddGridcellsForm(request.POST, pending_minions_list = pending_minions)
      if form.is_valid():
        #print 'form valid'
...
def add_a_gridcell_to_storage_pool(request):
  ...
  try:
    ...
    if d:
      ...
      raise Exception('Could not add the GRIDCell %s to the grid.'%gridcell_name)
...
def replace_gridcell(request):
  ...
  try:
    ...
    #Get info about all the volumes so we can find out which ones reside on this gridcell
    vil, err = gluster_volumes.get_basic_volume_info_all()
    if err:
      raise Exception(err)

    #Get the system info dict so we can display the potential list of replacement gridcells
    si, err = system_info.load_system_config()
    if err:
      raise Exception(err)
    if not si:
        ...
        if "node" not in request.POST or  "serial_number" not in request.POST:
          ...
          if step == "offline_disk":
            ...
            if not pool:
              ...
              ret, err = audit.audit("replace_disk_offline_disk", audit_str, request.META)
              if err:
                raise Exception(err)
            ...
            ret, err = audit.audit("replace_disk_scheduled", audit_str, request.META)
            if err:
              raise Exception(err)
...
def identify_gridcell(request):
  try:
    ...
    client = salt.client.LocalClient()
    blink_time = 255
    ret = client.cmd(gridcell_name,'cmd.run',['ipmitool chassis identify %s' %(blink_time)])
    #print ret
