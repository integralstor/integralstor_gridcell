...
from integralstor_utils import command, audit, cifs, lock, filesize
...
def volume_selector(request):
    ...
    try:
        ...
        if not vil:
            raise Exception(
                'No volumes created. Please create a volume before performing this action.')
        vol_list = []
        for v in vil:
            vol_list.append(v['name'])
        if request.method == 'GET':
            form = integral_view.forms.volume_management_forms.VolumeNameForm(
                vol_list=vol_list)
            ...
            form = integral_view.forms.volume_management_forms.VolumeNameForm(
                request.POST, vol_list=vol_list)
...
def view_volume(request):
    ...
    try:
        ...
        if not vol:
            raise Exception(
                "Could not locate information for volume %s" % vol_name)
        return_dict["vol"] = vol
...
def create_volume_dir(request):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            init['path'] = path
            form = integral_view.forms.volume_management_forms.CreateVolumeDirForm(
                initial=init)
            return_dict['form'] = form
            ...
            if not form.is_valid():
                ...
                cd = form.cleaned_data
                ret, err = gluster_gfapi.create_gluster_dir(
                    cd['vol_name'], (cd['path'] + cd['dir_name']), 0777)
                if err:
                    raise Exception(err)
                if ret:
...
def delete_volume(request):
    ...
    try:
        ...
        if shrs:
            raise Exception(
                "A volume can be removed only if all its underlying shares are removed. The share(s) with name(s) %s exists under volume. Please delete the share(s) and try again" % ','.join(shrs))
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if "cmd" not in request.POST or "vol_name" not in request.POST or "count" not in request.POST:
                raise Exception(
                    "Invalid request. Please use the menu options.")
...
def set_volume_options(request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if not vol_dict:
                raise Exception(
                    'Could not retrieve the information for volume %s' % vol_name)
            d = {}
            # Set default values
            d["auth_allow"] = '*'
            d["auth_reject"] = 'NONE'
            ...
            form = integral_view.forms.volume_management_forms.VolumeOptionsForm(
                initial=d)
            return_dict["form"] = form
...
def set_dir_quota(request):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            form = integral_view.forms.volume_management_forms.VolumeQuotaForm(
                request.POST)
            if not form.is_valid():
                return_dict["form"] = form
                return django.shortcuts.render_to_response('edit_volume_dir_quota.html', return_dict, context_instance=django.template.context.RequestContext(request))
            ...
            if not quota_enabled:
                res, err = gluster_quotas.change_quota_status(
                    vol_name, 'enable')
                if err:
                    raise Exception(err)
            ...
            if err:
                raise Exception(err)
            result_message = 'Successfully set quota for directory %s on volume %s to %s %s' % (
...
def change_quota_status(request):
    ...
    try:
        ...
        if request.method == 'GET' and action == 'disable':
            ...
            if err:
                raise Exception(err)
            result_message = 'Successfully %sd quota for volume %s' % (
                action, vol_name)
            return_dict['result_message'] = result_message
...
def create_snapshot(request):
    ...
    try:
        ...
        for v in vil:
            l.append(v["name"])
        if request.method == "GET":
            form = integral_view.forms.volume_management_forms.CreateSnapshotForm(
                vol_list=l)
            return_dict["form"] = form
            return django.shortcuts.render_to_response('create_snapshot.html', return_dict, context_instance=django.template.context.RequestContext(request))
        else:
            ...
            if "snapshot_name" not in request.POST:
                ...
                if d:
                    ...
                    if "op_status" in d:
                        ...
                        raise Exception(
                            "Could not detect the status of the snapshot deletion. Please try again.")
        ...
        if request.method == "GET":
            ...
            if "snapshot_name" not in request.POST:
                ...
                if d:
                    ...
                    if "op_status" in d:
                        if d["op_status"]["op_errno"] == 0:
                            return_dict["conf"] = "Successfully restored snapshot %s onto volume %s" % (
                        ...
                        raise Exception(
                            "Could not detect the status of the snapshot restoration. Please try again.")
...
def deactivate_snapshot(request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if "snapshot_name" not in request.POST:
                raise Exception(
                    "Snapshot name not specified. Please use the menu options.")
            elif "conf" not in request.POST:
                # Get a conf from the user before we proceed
                return_dict["snapshot_name"] = request.POST["snapshot_name"]
                ...
                if d:
                    ...
                    if "op_status" in d:
                        ...
                        raise Exception(
                            "Could not detect the status of the snapshot deactivation. Please try again.")
            ...
            if "snapshot_name" not in request.POST:
                ...
                if d:
                    ...
                    if "op_status" in d:
                        ...
                        raise Exception(
                            "Could not detect the status of the snapshot activation. Please try again.")
