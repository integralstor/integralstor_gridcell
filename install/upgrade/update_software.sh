...
echo "Entered IP is: " $input
read -t 30 -p "Please Press 'yes' to Confirm: " confirm
...
/tmp/updater.sh
read -t 30 -p "Successfully Updated to Latest software. Please Press 'yes' to restart the system now: " again
	if [ "$again" == "yes" ]; then
		reboot now
	else
