...
MAXBOOTSIZE=9			 			#Assign maximum size for OS disk u wanted to skip for Default Pool creation

#if [ "$MAXBOOTSIZE" -ge "10" ]; then
#	echo "Data disk size must be less than 10GB."
#exit 0
#fi
...
for DEV in "${block_devices[@]}" ; do
  if [ -b "/dev/$DEV" ] ; then
    ...
    GB=$(($SIZE/2**21))			
    #echo $GB $MAXBOOTSIZE
    if [ $GB -lt $MAXBOOTSIZE ] ; then			# skips the disk which is greater than the MAXBOOTSIZE 
