...
def configure_minion(*masters):
  try:
    ...
    #Record the new masters so that they can be used by other scripts like rc.local
...
def configure_ntp_slave(*masters):
  try:
    ...
    with open('/tmp/new_ntp_slave.conf', 'w') as f:
      ...
      f.write('# Dont serve time or stats to anyone else by default (more secure)\n')
      f.write('restrict default noquery nomodify\n')

      for master in masters:
        f.write('restrict %s mask 255.255.255.255 nomodify notrap noquery\n'%master)
