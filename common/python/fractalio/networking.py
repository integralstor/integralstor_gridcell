...
def change_hosts_file_entry(old_hostname, old_ip, new_hostname, new_ip):
  try:
    with open('/etc/hosts', 'r') as f:
      with open('/tmp/fractalio_newhosts', 'w') as f1:
        for line in f:
          if line.startswith('#'):
            continue
          if (old_hostname in line.strip()) or (old_ip in line.strip()):
            continue
          else:
            f1.write('%s\n'%line.strip())
        f1.write('%s %s %s.fractalio.lan\n'%(new_ip, new_hostname, new_hostname))
    shutil.move('/tmp/fractalio_newhosts', '/etc/hosts')
  except Exception, e:
    return -1
  else:
    return 0

def set_hostname(old_hostname, old_ip, new_hostname, new_ip):
  try:
    with open('/etc/sysconfig/network', 'w') as f:
      ...
      f.flush()
    f.close()
    r, rc = command.execute_with_rc('hostname %s'%new_hostname)
    if rc == 0:
      ...
      return -1
    rc = change_hosts_file_entry(old_hostname, old_ip, new_hostname, new_ip)
    return rc
  except Exception, e:
    return -1
