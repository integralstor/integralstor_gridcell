...
def load_iscsi_volumes_list(vil):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    il, err = db.read_multiple_rows("%s/integral_view_config.db"%db_path, "select * from iscsi_volumes")
    if err:
      raise Exception(err)
    if il:
      for i in il:
    ...
    return iscsi_volumes_list, None
...
def add_iscsi_volume(vol_name):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_volumes where vol_name=\'%s\'"%vol_name)
    if err:
      raise Exception(err)
    if d:
      return
...
def load_targets_list():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    iscsi_target_list, err = db.read_multiple_rows("%s/integral_view_config.db"%db_path, "select * from iscsi_targets")
    if err:
      raise Exception(err)
  except Exception, e:
    return None, 'Error loading ISCSI targets list : %s'%str(e)
  else:
    return iscsi_target_list, None
...
def load_target_info(index):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_targets where id=\'%d\'"%index)
    if err:
      raise Exception(err)
  except Exception, e:
...
def delete_target(id):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    ret, err = db.execute_iud([("%s/integral_view_config.db"%db_path, "delete from iscsi_targets where id = \'%d\'"%id,)])
    if err:
      raise Exception(err)
  except Exception, e:
...
def delete_all_targets():
  try:
    ...
    if err:
      raise Exception(err)
    ret, err = db.execute_iud("%s/integral_view_config.db"%db_path, [("delete from iscsi_targets",)])
    if err:
      raise Exception(err)
  except Exception, e:
...
def load_initiators_list():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    iscsi_initiator_list, err = db.read_multiple_rows("%s/integral_view_config.db"%db_path, "select * from iscsi_initiators")
    if err:
      raise Exception(err)
    ...
    return iscsi_initiator_list, None
  '''
...
def load_initiator_info(index):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_initiators where id=\'%d\'"%index)
    if err:
      raise Exception(err)
  except Exception, e:
    return None, ' : %s'%str(e)
  ...
  '''
  ret = None
  init_list, err = load_initiators_list()
...
def create_iscsi_initiator(initiators, auth_network, comment):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_initiators where auth_network = \'%s\' and initiators=\'%s\'"%(initiators.lower(), auth_network.lower()))
    if err:
      raise Exception(err)
    if d:
...
def delete_initiator(id):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    ret, err = db.execute_iud([("%s/integral_view_config.db"%db_path, "delete from iscsi_initiators where id = \'%d\'"%id,)])
    if err:
      raise Exception(err)
  except Exception, e:
...
def delete_all_initiators():
  try:
    ...
    if err:
      raise Exception(err)
    ret, err = db.execute_iud("%s/integral_view_config.db"%db_path, [("delete from iscsi_initiators",)])
    if err:
      raise Exception(err)
  except Exception, e:
...
def load_auth_access_group_list():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    iscsi_auth_access_group_list, err = db.read_multiple_rows("%s/integral_view_config.db"%db_path, "select * from iscsi_auth_access_groups")
  except Exception, e:
    return None, 'Error loading authorized access group list : %s'%str(e)
  else:
    return iscsi_auth_access_group_list, None
...
def load_auth_access_users_list():
  iscsi_auth_access_users_list = None
  try:
    db_path, err = common.get_db_path()
    if err:
      raise Exception(err)
    iscsi_auth_access_users_list, err = db.read_multiple_rows("%s/integral_view_config.db"%db_path, "select * from iscsi_auth_access_users")
    if err:
      raise Exception(err)
  except Exception, e:
    return None, 'Error loading authorized access users list : %s'%str(e)
  else:
    return iscsi_auth_access_users_list, None
  '''
...
def load_auth_access_users_info(auth_access_group_id):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    l, err = db.read_multiple_rows("%s/integral_view_config.db"%db_path, "select * from iscsi_auth_access_users where auth_access_group_id = \'%d\'"%auth_access_group_id)
    if err:
      raise Exception(err)
  except Exception, e:
  ...
  ret = None
  l = []
  aa_list, err = load_auth_access_users_list()
  for aa in aa_list:
...
def load_auth_access_user_info(user_id):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_auth_access_users where id = \'%d\'"%user_id)
  except Exception, e:
...
def create_auth_access_group():
  ...
  try:
    ...
    rowid, err = db.execute_iud("%s/integral_view_config.db"%db_path, cl, True)
  ...
  '''
  l, err = load_auth_access_group_list()
  max_id = 0
  for i in l:
    if i["id"] > max_id:
      max_id = i["id"]
...
def create_auth_access_user(auth_access_group_id, user, secret):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_auth_access_users where auth_access_group_id = \'%d\' and user = \'%s\'"%(auth_access_group_id, user))
    if err:
      raise Exception(err)
    if d:
  ...
  '''
  l, err = load_auth_access_users_list()
  max_user_id = 0
  for i in l:
...
def delete_auth_access_group(id):
  ...
  try:
    ...
    return False, 'Error deleting authorized access group : %s'%str(e)
  else:
    return True, None
...
def delete_all_auth_access_groups():
  try:
    ...
    if err:
      raise Exception(err)
    ret, err = db.execute_iud("%s/integral_view_config.db"%db_path, [("delete from iscsi_auth_access_groups",)])
    if err:
      raise Exception(err)
  except Exception, e:
    return False, 'Error deleting all authorized access groups : %s'%str(e)
  else:
    return True, None
...
def delete_all_auth_access_users():
  try:
    ...
    if err:
      raise Exception(err)
    ret, err = db.execute_iud("%s/integral_view_config.db"%db_path, [("delete from iscsi_auth_access_users",)])
    if err:
      raise Exception(err)
  except Exception, e:
...
def load_global_target_conf():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    igt, err  = db.read_single_row("%s/integral_view_config.db"%db_path, "select * from iscsi_global_target_conf where id = \'1\'")
    if err:
      raise Exception(err)
  except Exception, e:
...
def generate_global(fh, dict):
  try:
    ...
    return False, "Error generating configuration file's global section : %s"%str(e)
  else:
    return True, None
...
def generate_unit_control(fh):
  try:
    ...
    return False, "Error generating configuration file's unit control section : %s"%str(e)
  else:
    return True, None
...
def generate_portal(fh):
  try:
    ...
    return False, "Error generating configuration file's portal section : %s"%str(e)
  else:
    return True, None
...
def generate_initiator(fh, dict):
  try:
    ...
    return False, "Error generating configuration file's initiator section : %s"%str(e)
  else:
    return True, None
...
def generate_logical_unit(fh, id, vol_name, target_name, init_group_id, auth_method, auth_group_id, queue_depth, target_alias, lun_size):
  try:
    ...
    fh.write ("  #TargetName iqn.2014-15.in.gridcell.istgt:%s \n"%vol_name)
    ...
    return False, "Error generating configuration file's LUN section : %s"%str(e)
  else:
    return True, None
...
def generate_istgt_conf():
  try:
    with open("istgt.conf", "w") as fh:
      ...
      d, err = load_global_target_conf()
      if err:
        raise Exception(err)
      ret, err = generate_global(fh, d)
      if err:
        raise Exception(err)
      ret, err = generate_unit_control(fh)
      if err:
        raise Exception(err)
      ret, err = generate_portal(fh)
      if err:
        raise Exception(err)
      il, err = load_initiators_list()
      if err:
        raise Exception(err)
      if il:
        for i in il:
          ret, err = generate_initiator(fh, i)
          if err:
            raise Exception(err)
      tl, err = load_targets_list()
      if err:
        raise Exception(err)
      if tl:
        for t in tl:
          ret, err = generate_logical_unit(fh, t["id"], t["vol_name"],  t["target_name"], t["init_group_id"], t["auth_method"], t["auth_group_id"], t["queue_depth"], t["target_alias"], t["lun_size"])
          if err:
            raise Exception(err)

  except Exception, e:
    return False, 'Error generating configuration file : %s'%str(e)
  else:
    return True, None
