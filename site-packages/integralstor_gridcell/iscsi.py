...
def load_targets_list():
    ...
    try:
        ...
        if err:
            raise Exception(err)
        iscsi_target_list, err = db.read_multiple_rows(
...
def load_initiators_list():
    ...
    try:
        ...
        if err:
            raise Exception(err)
        iscsi_initiator_list, err = db.read_multiple_rows(
...
def load_auth_access_group_list():
    ...
    try:
        ...
        if err:
            raise Exception(err)
        iscsi_auth_access_group_list, err = db.read_multiple_rows(
...
def load_auth_access_users_list():
    ...
    try:
        ...
        if err:
            raise Exception(err)
        iscsi_auth_access_users_list, err = db.read_multiple_rows(
            db_path, "select * from iscsi_auth_access_users")
        if err:
            raise Exception(err)
    except Exception, e:
...
def delete_auth_access_group(id):
    ...
    try:
        ...
        if err:
            raise Exception(err)
        cl = [("delete from iscsi_auth_access_groups where id = \'%d\'" % id,),
              ("delete from iscsi_auth_access_users where auth_access_group_id = \'%d\'" % id,)]
        ret, err = db.execute_iud(db_path, cl)
        if err:
            raise Exception(err)
    except Exception, e:
...
def delete_all_auth_access_groups():
    try:
        ...
        if err:
            raise Exception(err)
        ret, err = db.execute_iud(
            db_path, [("delete from iscsi_auth_access_groups",)])
        if err:
            raise Exception(err)
    except Exception, e:
...
def generate_istgt_conf():
    try:
        with open("istgt.conf", "w") as fh:
            ...
            if tl:
                for t in tl:
                    ret, err = generate_logical_unit(fh, t["id"], t["vol_name"],  t["target_name"], t["init_group_id"],
                                                     t["auth_method"], t["auth_group_id"], t["queue_depth"], t["target_alias"], t["lun_size"])
                    if err:
                        raise Exception(err)
