...
def create_replace_command_file(si, vol_list, src_node, dest_node):
    ...
    try:
        ...
        batch_dir, err = config.get_batch_files_path()
        if err:
            raise Exception(err)
        if not os.path.exists(batch_dir):
            os.mkdir(batch_dir)

        # This is the dict which will form the basis of the JSON that will be
        # stored in the batch file.
        data = {}
        data["title"] = "Replacing GRIDCell %s with GRIDCell %s" % (
            src_node, dest_node)
        ...
        if vol_list:
                ...
                if vd and ('bricks' in vd and vd['bricks']):
                    for brick_list in vd['bricks']:
                        for brick in brick_list:
                            ...
                            brick_components, err = volume_info.get_components(
                                brick)
                            if err:
                                raise Exception(err)
                            ...
                            if brick_components['host'] != src_node:
                                continue
                            brick_path = '/%s/%s/%s' % (
                                brick_components['pool'], brick_components['dataset'], tv)
                            if vd["type"] == "Distribute":
                                d = {}
                                #c = "/usr/sbin/gluster volume replace-brick %s %s:%s %s:%s"%(tv, src_node, brick_path, dest_node, brick_path)
                                c = "/usr/sbin/gluster volume add-brick %s %s:%s force --xml" % (
                                    tv, dest_node, brick_path)
                                d["type"] = "add_brick"
                                d["desc"] = "Adding volume storage in GRIDCell %s for volume %s" % (
                                    dest_node, tv)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
                                d = {}
                                c = "/usr/sbin/gluster --mode=script volume remove-brick %s %s:%s start --xml" % (
                                    tv, src_node, brick_path)
                                d["type"] = "remove_brick_start"
                                d["desc"] = "Migrating volume storage from GRIDCell %s for volume %s start" % (
                                    src_node, tv)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
                                d = {}
                                c = "/usr/sbin/gluster volume remove-brick %s %s:%s status --xml" % (
                                    tv, src_node, brick_path)
                                d["type"] = "remove_brick_status"
                                d["desc"] = "Migrating volume storage from GRIDCell %s for volume %s status" % (
                                    src_node, tv)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
                                d = {}
                                c = "/usr/sbin/gluster --mode=script volume remove-brick %s %s:%s commit --xml" % (
                                    tv, src_node, brick_path)
                                d["type"] = "remove_brick_commit"
                                d["desc"] = "Migrating volume storage from GRIDCell %s for volume %s commit" % (
                                    src_node, tv)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
                                ...
                                d = {}
                                c = "/usr/sbin/gluster volume replace-brick %s %s:%s %s:%s commit force --xml" % (
                                    tv, src_node, brick_path, dest_node, brick_path)
                                d["type"] = "replace_brick_commit"
                                d["desc"] = "Replacing storage location for volume %s from GRIDCell %s to GRIDCell %s" % (
                                    tv, src_node, dest_node)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
                                d = {}
                                ...
                                d["desc"] = "Migrating volume data from GRIDCell %s for volume %s start" % (
                                    src_node, tv)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
                                ...
                                d["desc"] = "Migrating volume data from GRIDCell %s for volume %s info" % (
                                    src_node, tv)
                                d["command"] = c
                                d["status_code"] = 0
                                data["command_list"].append(d)
            ...
            file_name = "%s_%d" % (time.strftime(
                "bp_replace_node_%b_%d_%Y_%H_%M_%S", t), int(time.time()))
...
def create_rebalance_command_file(vol_name):
    ...
    try:
        ...
        data["initiate_time"] = time.strftime("%a %b %d %H:%M:%S %Y", t)
        file_name = "%s_%d" % (time.strftime(
            "bp_volume_rebalance_%b_%d_%Y_%H_%M_%S", t), int(time.time()))
        full_file_name = "%s/%s" % (batch_dir, file_name)
        data["status_url"] = "/view_batch_process?file_name=%s" % file_name
