...
def create_replace_command_file(si, vil, src_node, dest_node):
  ...
  try:
    ...
    if tvl:
      for tv in tvl:
        vd, err = volume_info.get_volume_info(vil, tv)
        if err:
          raise Exception(err)
        if vd and ('bricks' in vd and vd['bricks']):
          brick1 = vd['bricks'][0]
          brick_components, err = volume_info.get_components(brick1)
          if err:
            raise Exception(err)
          brick_path = '/%s/%s/%s'%(brick_components['pool'], brick_components['ondisk_storage'], tv)
        else:
          raise Exception('Error retrieving volumd information/brick paths')
    ...
    None, 'Error creating the replace command batch file : %s'%str(e)
  else:
    return d, None
