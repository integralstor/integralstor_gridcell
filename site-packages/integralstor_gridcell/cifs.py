...
def _generate_local_share(f):
    try:
        ret, err = cifs_common.generate_share_header(
            f, 'integralstor_local_storage')
        if err:
            raise Exception(err)
...
def _generate_share_section(f, d, extra_share_param_lines=None):
    ...
    try:
        ...
        ret, err = cifs_common.generate_common_share_body(f, d['workgroup'], d['path'], d['read_only'], d['browseable'],
                                                          d['guest_ok'], d['user_list'], d['group_list'], d['comment'], d['auth_method'], extra_share_param_lines)
        if err:
            raise Exception(err)
            ...
            if shl:
                for share in shl:
                    ...
                    if not share["guest_ok"]:
                        vul, err = cifs_common.load_valid_users_list(
                            share["share_id"])
                        if err:
                            raise Exception(err)
                        # print 'vul is ', vul
