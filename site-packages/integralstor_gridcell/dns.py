...
# Controls whether to use the hosts file or named. Valid strings are
# 'hosts' or 'named'
dns_mode = 'hosts'
...
def unpopulate_hosts_file(client, gridcell_name, mapping_dict):
    """Removes all hostname/IP combination to the hosts file on a specific gridcell.

    client - salt's client handle.
    gridcell_name - the gridcell whose hosts file needs to be unpopulated.
    mapping_dict - The hostname/ip mapping dict to be unpopulated.
    """
    try:
        successful_dict = {}
        failures = False
        for hn, ip in mapping_dict.items():
            if 'integralstor.lan' not in hn:
                ...
                r1 = client.cmd(gridcell_name, 'hosts.rm_host', [ip, hn])
            if r1:
                for hn, result in r1.items():
                    if result is not None:
                        if result:
                ...
                raise Exception('Could not unpopulate the hosts file')
    except Exception, e:
        return False, "Error unpopulating the hosts file on GRIDCell %s : %s" % (gridcell_name, str(e))
    else:
        return True, None
...
def add_to_dns(client, hn, ip):
    ...
    try:
        no_op = False
        if dns_mode == 'named':
            # Dont add the primary and secondary because they are already there
            ...
            if hn in ['gridcell-pri.integralstor.lan', 'gridcell-sec.integralstor.lan', 'gridcell-pri', 'gridcell-sec']:
                no_op = True
                pass
            r1 = client.cmd('roles:primary', 'ddns.add_host', [
                            'integralstor.lan', hn, 86400, ip], expr_form='grain', timeout=180)
        else:
            if 'integralstor.lan' not in hn:
                r1 = client.cmd('*', 'hosts.add_host',
                                [ip, '%s.integralstor.lan' % hn])
            else:
...
def remove_from_dns(client, hn):
    ...
    try:
        ...
        if dns_mode == 'named':
            # Dont remove the primary and secondary because they are always
            # needed.
            if hn in ['gridcell-pri.integralstor.lan', 'gridcell-sec.integralstor.lan', 'gridcell-pri', 'gridcell-sec']:
                raise Exception(
                    'Cannot remove the primary or secondary GRIDCells from DNS.')
            r1 = client.cmd('roles:primary', 'ddns.delete_host', [
                            'integralstor.lan', hn], expr_form='grain', timeout=180)
            if not r1:
                raise Exception("GRIDCell %s" % hn)
        else:
            ip = None
            if 'integralstor.lan' not in hn:
                r1 = client.cmd('*', 'hosts.get_ip',
                                ['%s.integralstor.lan' % hn])
            else:
                r1 = client.cmd('*', 'hosts.get_ip', [hn])
            if not r1:
                raise Exception("GRIDCell %s" % hn)
            else:
                for key, value in r1.items():
            ...
            if ip:
                print 'removing ', ip
                if 'integralstor.lan' not in hn:
                    r1 = client.cmd('*', 'hosts.rm_host',
                                    [ip, '%s.integralstor.lan' % hn])
                else:
                    r1 = client.cmd('*', 'hosts.rm_host', [ip, hn])
                err_list = []
                for key, value in r1.items():
                    if value is not None and value == False:
                        err_list.append(
                            'Error removing hostname entry "%s" from GRIDCell %s' % (hn, key))
                if err_list:
                    raise Exception(
                        'The following errors occurred : %s. Please resynchronize the hosts file' % '.'.join(err_list))
            else:
