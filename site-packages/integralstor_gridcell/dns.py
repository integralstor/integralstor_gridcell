  ...
  try:
    ...
    for hn, ip in mapping_dict.items():
      if 'integralstor.lan' not in hn:
        r1 = client.cmd(gridcell_name, 'hosts.add_host', [ip, '%s.integralstor.lan'%hn])
      else:
    ...
    if failures:
      #If any items successfully added them remove them so re roll back to the original state
      if successful_dict:
        for hn, ip in successful_dict.items():
          if 'integralstor.lan' not in hn:
            r1 = client.cmd(gridcell_name, 'hosts.rm_host', [ip, '%s.integralstor.lan'%hn])
          else:
