...
def get_volume_status(vol_name, vol_info_dict = None):
  ...
  try:
    ...
    d, err = xml_parse.run_gluster_command("/usr/sbin/gluster volume status %s detail --xml"%vol_name)
    if err:
      raise Exception(err)
    ...
    #Now calculate the size statistics for the volume from individual bricks
    size_total = 0
    size_free = 0
    if vol_info_dict["replica_count"] > 1:
      ...
      if num_up == 0:
        ...
        if max(replica_set_status) == vol_info_dict["replica_count"]:
          ...
          num_more = vol_info_dict["replica_count"] - max(replica_set_status)
      ...
      if num_down > 0:
        ...
        return_dict["data_access_status"] = "Healthy"
        return_dict["data_access_status_code"] = 0
    return_dict["size_total"] = filesize.naturalsize(size_total, binary=True)
    return_dict["size_used"] = filesize.naturalsize(size_total-size_free, binary=True)
    return_dict["size_free"] = filesize.naturalsize(size_free, binary=True)
    #print size_total-size_free
    #print (size_total-size_free)/float(size_total)
    if size_total > 0:
      return_dict["size_used_percent"] = int(((size_total-size_free)/float(size_total)) * 100)
    else:
...
def get_volume_process_status(vol_name, vol_info_dict = None, vol_status_dict = None):
  ...
  try:
    ...
    d, err = xml_parse.run_gluster_command("/usr/sbin/gluster volume status %s --xml"%vol_name)
    if err:
      raise Exception(err)

    return_dict, err = xml_parse.get_volume_process_status(d['root'], vol_info_dict, vol_status_dict)
    if err:
      raise Exception(err)
  except Exception, e:
      ...
      for o in vol_info_dict["options"]:
        if "features.quota" == o["name"]:
          if o["value"]  == "on" and vol_info_dict['status'] == 1:
            # Now get the status of the self heal and NFS servers for each node
            d, err = xml_parse.run_gluster_command('gluster volume quota %s list --xml'%(vol_name))
            if err:
              #print d
            ...
            if not no_quotas_set:
              return_dict, err = xml_parse.get_vol_quotas(d['root']) 
              if err:
                raise Exception(err)
  except Exception, e:
...
def get_complete_volume_info(vol_name, vol_info_dict = None):
  ...
  try:
    ...
    vol_status_dict = {}
    if vol_info_dict['status'] == 1:
      vol_status_dict, err = get_volume_status(vol_info_dict['name'], vol_info_dict)
      if err:
        raise Exception(err)
    ...
    if vol_info_dict['status'] == 1:
      ...
      vol_process_dict, err = get_volume_process_status(vol_name, vol_info_dict, vol_status_dict)
      if err:
        raise Exception(err)
      ...
      for br in return_dict['brick_status']:
        if br in vol_process_dict['brick_status']:
          return_dict['brick_status'][br].update(vol_process_dict['brick_status'][br])
  ...
  try:
    ...
    if vl:
      for vol_info_dict in vl:
        ...
        rd, err = get_complete_volume_info(vol_info_dict['name'], vol_info_dict)
        if err:
          raise Exception(err)
...
def set_volume_options(cd):
  ...
  try:
    ...
    if _auth_allow != auth_allow:
      ...
      if err:
        raise Exception(err)
      d['audit_str'] = "Setting option for permitted access IP addresses for %s to \'%s\'"%(vol_name, auth_allow)
      ret_list.append(d)
    ...
    if _auth_reject != auth_reject:
      ...
      if err:
        raise Exception(err)
      d['audit_str'] = "Setting option for denied access IP addresses for %s to \'%s\'"%(vol_name, auth_reject)          
      ret_list.append(d)

    if _readonly != readonly:
      d, err = _set_volume_option(vol_name, "features.read-only", readonly)
      if err:
        raise Exception(err)
      d['audit_str'] = "Setting readonly mount access(for all access methods) for %s to \'%s\'"%(vol_name, readonly)
      ret_list.append(d)
    ...
    if readonly == "off":
      ...
      if _nfs_disable != nfs_disable:
        ...
        if err:
          raise Exception(err)
        d['audit_str'] = "Setting NFS disable for %s to \'%s\'"%(vol_name, p)
        ret_list.append(d)

      if not nfs_disable:
        #print "in"
        if nfs_volume_access and _nfs_volume_access != nfs_volume_access:
          d, err = _set_volume_option(vol_name, "nfs.volume-access", nfs_volume_access)
          if err:
            raise Exception(err)
          d['audit_str'] = "Setting NFS access type for %s to \'%s\'"%(vol_name, nfs_volume_access)
          ret_list.append(d)
      ...
      if _enable_worm != enable_worm:
        ...
        if err:
          raise Exception(err)
        d['audit_str'] = "Setting feature WORM for %s to \'%s\'"%(vol_name, p)
        ret_list.append(d)
...
def build_create_or_expand_volume_command(cmd, si, anl, vol_type, ondisk_storage, repl_count, vol_name):
  ...
  try:
    ...
    if not anl:
      raise Exception('No available GRIDCells to expand/create the volume.')
...
def _get_allowable_node_list(si, vol_name = None):
  ...
  try:
    for hostname in si.keys():
      #Volumes can only be placed on nodes that are ok and are part of the storage pool
...
def build_expand_volume_command(vol_info_dict, si):
  ...
  try:
    ...
    return_dict, err = build_create_or_expand_volume_command(cmd, si, anl, vol_type, ondisk_storage, repl_count, vol_info_dict["name"])
    if err:
      raise Exception(err)
...
def build_create_volume_command(vol_name, vol_type, ondisk_storage, repl_count, transport, si):
  ...
  try:
    ...
    d, err = build_create_or_expand_volume_command(cmd, si, anl, vol_type, ondisk_storage, repl_count, vol_name)
    if err:
      raise Exception(err)
    if "cmd" in d:
      d["cmd"] = d["cmd"] + " --xml"
      ...
      for vsda_tree in vsda_tree_heads:
        ...
        if vol["replica_count"] > 1:
          ...
          if num_down > 0:
            ...
            vol["data_access_status"] = "Healthy"
            vol["data_access_status_code"] = 0
        vol["size_total"] = filesize.naturalsize(size_total, binary=True)
        vol["size_used"] = filesize.naturalsize(size_total-size_free, binary=True)
        vol["size_free"] = filesize.naturalsize(size_free, binary=True)
        #print size_total-size_free
        #print (size_total-size_free)/float(size_total)
        if size_total > 0:
