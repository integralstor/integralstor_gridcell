...
from integralstor_gridcell import xml_parse, gluster_commands
        ...
        if "options" in v :
          for o in v["options"]:
            if "features.quota" == o["name"]:
              if o["value"]  == "on" and v['status'] == 1:
                v["quotas"], err = gluster_commands.get_volume_quotas(v["name"]) 
                #print v['name'], v['quotas'], err
                if err:
                  raise Exception(err)
  except Exception, e:
...
def _get_volume_list():
  ...
  try:
    ...
    if err:
      raise Exception(err)
    d, err = gluster_commands.run_gluster_command("/usr/sbin/gluster volume info all --xml", "%s/b.xml"%devel_files_path, "Getting volume info")
    if err:
      raise Exception(err)
    ...
    if err:
      raise Exception(err)
    vl, err = xml_parse.get_volume_list(via_tree, admin_vol_name)
    if err:
      raise Exception(err)
    ...
    if vl:
      ...
      d, err = gluster_commands.run_gluster_command("/usr/sbin/gluster volume status all detail --xml", "%s/b.xml"%devel_files_path, "Getting volume info")
      if err:
        raise Exception(err)
      if not d or  "op_status" not in d or ("op_status" in d and d["op_status"]["op_ret"] != 0):
        err = "Error getting the volume infomation : "
      ...
      for vsda_tree in vsda_tree_heads:
        _vol_name, err = xml_parse.get_text(vsda_tree, "volName")
        if err:
      ...
      d, err = gluster_commands.run_gluster_command("/usr/sbin/gluster volume status all --xml", "%s/volume_status.xml"%devel_files_path, "Getting volume status")
      if err:
        raise Exception(err)
      if not d or  "op_status" not in d or ("op_status" in d and d["op_status"]["op_ret"] != 0):
      ...
      for vsd_tree in vsd_tree_heads:
        _vol_name, err = xml_parse.get_text(vsd_tree, "volName")
        if err:
          raise Exception('Error reading volume name from volume status detail XML')
        for vol in vl:
          if vol['name'] == _vol_name:
    ...
    '''
      #Refactored code above replaces this code in order to minimize the number of gluster calls

    if vl:
      for vol in vl:
        if vol["status"] != 1:
          continue      
        d, err = gluster_commands.run_gluster_command("/usr/sbin/gluster volume status %s detail --xml"%vol["name"], "%s/volume_status_detail.xml"%devel_files_path, "Getting volume status details")
        if err:
          raise Exception(err)
        if not d or  "op_status" not in d or ("op_status" in d and d["op_status"]["op_ret"] != 0):
        ...
        # Now get the status of the self heal and NFS servers for each node
        d, err = gluster_commands.run_gluster_command("/usr/sbin/gluster volume status %s --xml"%vol["name"], "%s/volume_status.xml"%devel_files_path, "Getting volume status")
        if err:
          raise Exception(err)
        if not d or  "op_status" not in d or ("op_status" in d and d["op_status"]["op_ret"] != 0):
          err = "Error getting the volume status : "
...
def get_volumes_on_node(hostname, vil):
  ...
  try:
    ...
    if vil:
      vol_list = []
      for vol in vil:
        bl, err = get_brick_hostname_list(vol)
        if err:
          raise Exception(err)
...
def get_snapshots(vol_name):
  ...
  try:
    ...
    dummy_command = "%s/view_snapshots.xml"%devel_files_path
    d, err = gluster_commands.run_gluster_command(prod_command, dummy_command, "Starting volume %s"%vol_name)
    if err:
      raise Exception(err)
    if d:
...
def set_volume_options(cd):
  ...
  try:
    ...
    vol_dict, err = get_volume_info(None, vol_name)
    if err:
      raise Exception(err)
    ...
    if _auth_allow != auth_allow:
      d, err = _set_volume_option(vol_name, "auth.allow", auth_allow, "Setting option for permitted access IP addresses for %s to \'%s\'"%(vol_name, auth_allow))
      if err:
        raise Exception(err)
      ret_list.append(d)
    if _auth_reject != auth_reject:
      d, err = _set_volume_option(vol_name, "auth.reject", auth_reject, "Setting option for denied access IP addresses for %s to \'%s\'"%(vol_name, auth_reject))
      if err:
        raise Exception(err)
      ret_list.append(d)
    if _readonly != readonly:
      d, err = _set_volume_option(vol_name, "features.read-only", readonly, "Setting readonly mount access(for all access methods) for %s to \'%s\'"%(vol_name, readonly))
      if err:
        raise Exception(err)
      ret_list.append(d)
    if readonly == "off":
      ...
      if _nfs_disable != nfs_disable:
        if nfs_disable:
          ...
          p = "off"
        d, err = _set_volume_option(vol_name, "nfs.disable", p, "Setting NFS disable for %s to \'%s\'"%(vol_name, p))
        if err:
          raise Exception(err)
        ret_list.append(d)
      if not nfs_disable:
        ...
        if nfs_volume_access and _nfs_volume_access != nfs_volume_access:
          d, err = _set_volume_option(vol_name, "nfs.volume-access", nfs_volume_access, "Setting NFS access type for %s to \'%s\'"%(vol_name, nfs_volume_access))
          if err:
            raise Exception(err)
          ret_list.append(d)
      if _enable_worm != enable_worm:
        if enable_worm:
          ...
          p = "disable"
        d, err = _set_volume_option(vol_name, "features.worm", p, "Setting feature WORM for %s to \'%s\'"%(vol_name, p))
        if err:
          raise Exception(err)
        ret_list.append(d)
  except Exception, e:
...
def _set_volume_option(vol_name, option, value, display_command):
  ...
  try:
    ...
    dummy_command = "%s/set_vol_options.xml"%devel_files_path
    d, err = gluster_commands.run_gluster_command(prod_command, dummy_command, display_command)
    if err:
      raise Exception(err)
  except Exception, e:
