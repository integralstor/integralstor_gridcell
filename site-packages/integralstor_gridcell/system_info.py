    ...
    try:
        ...
        for hostname in si.keys():
            if not si[hostname]["in_cluster"]:
                # Not in the trusted storage pool so no need for any special
                # replacement.
                continue

            if (si[hostname]["node_status"] != 0) or (si[hostname]["volume_list"]):
                # Cant replace primary/secondary by the usual method so avoid
                # them.
...
def raid_enabled():
    # Enabled by default now. We need to change this to read from the config
    # db which in turn will be updated at the time of first config
    return True, None
