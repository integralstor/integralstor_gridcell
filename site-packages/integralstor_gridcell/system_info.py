    ...
    for k in d.keys():
      ...
      if peer_list:
        for peer in peer_list:
          #peer hostname could be hostname of IP so need to check both
          if k == peer["hostname"] or ('inet' in d[k]['interfaces']['bond0'] and d[k]['interfaces']['bond0']['inet'][0]['address'] == peer['hostname']):
            d[k]["in_cluster"] = True
            d[k]["cluster_status"] = int(peer["status"])
            d[k]["cluster_status_str"] = peer["status_str"]
        #if k in peer_list:
        #  d[k]["in_cluster"] = True
      d[k]["volume_list"], err = volume_info.get_volumes_on_node(k, vil)
      if err:
        raise Exception(err)
  except Exception, e:
