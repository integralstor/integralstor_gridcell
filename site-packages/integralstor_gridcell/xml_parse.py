    ...
    try:
        ...
        if nodes:
            for node in nodes:
                ...
                # This code covers some corner case in gluster's xml output,
                # forgot what it is!!!
                if node.find('./node'):
                    continue
            ...
            for node in nodes:
                ...
                for br in vol_status["brick_status"].keys():
                    ...
                    if h == path:
                        ...
                        break
                if found:
                    # br now holds the brick for which we will update the nfs
                    # and self heal status
                    if br not in return_dict['brick_status'].keys():
        ...
        if t:
            for q in t:
                ...
                if err:
                    raise Exception(err)
                d["hard_limit"] = val
                d['hard_limit_human_readable'] = filesize.naturalsize(
                    val, binary=True)
                ...
                if err:
                    raise Exception(err)
                d["used_space"] = val
                d['used_space_human_readable'] = filesize.naturalsize(
                    val, binary=True)
                ...
                if err:
                    raise Exception(err)
                d["avail_space"] = val
                d['avail_space_human_readable'] = filesize.naturalsize(
                    val, binary=True)
