...
def can_remove_gridcell_from_storage_pool(hostname, si = None):
  ...
  try:
    ...
    if not si:
      ...
      if err:
        raise Exception(err)
    localhost = socket.getfqdn().strip()
    if hostname != localhost and (hostname not in ['gridcell-sec.integralstor.lan', 'gridcell-pri.integralstor.lan']) and si[hostname]["in_cluster"] and (not si[hostname]["volume_list"]):
      ret = True
  except Exception, e:
