  ...
  try :
    ...
    if si[hostname]["in_cluster"] and hostname != localhost:
      ...
      if d and ("op_status" in d) and ("op_ret" in d["op_status"]) and (d["op_status"]["op_ret"] == 0):
        ...
        #rc, err = ctdb.remove_from_nodes_file(None, [ip])
        #if err:
        #  raise Exception(err)
        #if not rc:
...
def build_expand_volume_command(vol, si):
  ...
  try:
    ...
    # This check is no longer required for the current hardware
    #if len(anl) < 2:
...
def set_volume_dir_quota(vol_name, dir, limit, unit):
  ...
  try:
    prod_command = 'gluster volume quota %s limit-usage %s %s%s --xml'%(vol_name, dir, limit, unit)
    dummy_command = "%s/set_quota.xml"%devel_files_path
    #print prod_command
    d, err = run_gluster_command(prod_command, dummy_command, "Setting quota for directory %s in volume %s to %s %s"%(dir, vol_name, limit, unit))
    if d and ("op_status" in d) and d["op_status"]["op_ret"] == 0:
      pass
    else:
      err = " Quota failure."
...
def remove_volume_dir_quota(vol_name, dir):
  ...
  try:
    prod_command = 'gluster volume quota %s remove %s --xml'%(vol_name, dir)
    dummy_command = "%s/set_quota.xml"%devel_files_path
    #print prod_command
    d, err = run_gluster_command(prod_command, dummy_command, "Removing quota for directory %s in volume %s"%(dir, vol_name))
    if d and ("op_status" in d) and d["op_status"]["op_ret"] == 0:
      pass
    else:
      err = " Quota failure."
...
def change_quota_status(vol_name, action):
  ...
  try:
    dummy_command = "%s/set_quota.xml"%devel_files_path
    prod_command = 'gluster --mode=script volume quota %s %s --xml'%(vol_name, action)
    #print prod_command
    d, err = run_gluster_command(prod_command, dummy_command, "%sing quota for volume %s"%(action, vol_name))
    if d and ("op_status" in d) and d["op_status"]["op_ret"] == 0:
      pass
    else:
      err = " Quota failure."
      ...
      raise Exception(err)
    if action == 'enable':
      prod_command = 'gluster volume set %s quota-deem-statfs on --xml'%vol_name
      d, err = run_gluster_command(prod_command, dummy_command, "%sing quota for volume %s"%(action, vol_name))
      if err:
        raise Exception(err)
