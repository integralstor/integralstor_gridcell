...
def link_kerberos_file(client, hostname_list):
  ...
  try :
    ...
    r1 = client.cmd(hostname_list, 'cmd.run_all', ['rm /etc/krb5.conf'], expr_form='list')
...
def start_or_stop_services(client, hostname_list, action):
  ...
  try :
    ...
    r1 = client.cmd(hostname_list, 'cmd.run_all', ['service smd %s'%action], expr_form='list')
    #print r1
    if r1:
      for node, ret in r1.items():
        #print ret
        if ret["retcode"] != 0:
          errors = "Error %sing smd on %s"%(action, node)
          raise Exception(errors)
...
def add_a_gridcell_to_storage_pool(si, hostname):
  ...
  try:
    ...
    if err:
      raise Exception(err)
    r2 = client.cmd(hostname, 'cmd.run_all', ['rm /etc/rc.local'])
    r2 = client.cmd(hostname, 'cmd.run_all', ['rm /etc/rc.d/rc.local'])
