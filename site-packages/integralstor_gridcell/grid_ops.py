...
from integralstor_utils import command, audit, config, networking
...
def _regenerate_manifest_and_status(first_time=False):
    ...
    try:
        if first_time:
            ss_path, err = config.get_tmp_path()
            if err:
                raise Exception(err)
        else:
        ...
        scripts_path, err = config.get_python_scripts_path()
        if err:
            raise Exception(err)
...
def restart_minions(client, minion_list):
    ...
    try:
        r1 = client.cmd(minion_list, 'cmd.run', [
                        'echo service salt-minion restart | at now + 1 minute'], expr_form='list')
    except Exception, e:
...
def delete_salt_key(hostname):
    ...
    try:
        ...
        # The host continues to respond to requests even after the delete.
        # Sigh! So need to loop until it stops..
        client = salt.client.LocalClient()
            ...
            for minion in minions:
                ...
                if not rc:
                    if err:
                        ...
                        raise Exception(
                            "Error deleting salt key : Unknown error occurred")
    except Exception, e:
    ...
    try:
        ...
        if r:
            if 'ip_interfaces' in r[m] and r[m]['ip_interfaces']['bond0']:
                ...
                raise Exception(
                    "The IP address for bond0 in GRIDCell %s does not seem to have been configured." % m)
        else:
...
def get_minion_ips(client, hostname_list):
    ...
    mapping_dict = {}
    try:
        r = client.cmd(hostname_list, 'grains.item', [
                       'ip_interfaces'], expr_form='list')
        # print r
        if r:
            for hostname, interface_info in r.items():
                if 'ip_interfaces' in interface_info and interface_info['ip_interfaces']['bond0']:
                    ...
                    raise Exception(
                        "The IP address for bond0 in GRIDCell %s does not seem to have been configured." % hostname)
        else:
...
def link_kerberos_file(client, hostname_list):
    ...
    try:
        ...
        # print "Removing original krb5.conf file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'rm -f /etc/krb5.conf'], expr_form='list')
        # print "Removed original krb5.conf file on GRIDCell
        # %s."%','.join(hostname_list)

        # print "Linking the krb5.conf file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'ln -s %s/lock/krb5.conf /etc/krb5.conf' % config_dir], expr_form='list')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
        ...
        # print "Linked the krb5.conf file on GRIDCell
        # %s."%','.join(hostname_list)
...
def place_ctdb_files(client, hostname_list):
    ...
    try:
        ...
        # print "Removing original CTDB config file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'rm -f /etc/sysconfig/ctdb'], expr_form='list')
        # print "Removed original CTDB config file on GRIDCell
        # %s."%','.join(hostname_list)

        # print "Placing the CTDB config file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'cp %s/lock/ctdb /etc/sysconfig/ctdb' % config_dir], expr_form='list')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
                    ...
                    raise Exception(errors)
        # print "Placed the CTDB config file on GRIDCell
        # %s."%','.join(hostname_list)

        # print "Removing original CTDB nodes file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'rm -f /etc/ctdb/nodes'], expr_form='list')
        # print "Removed original CTDB nodes file on GRIDCell
        # %s."%','.join(hostname_list)

        # print "Placing the CTDB nodes file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'cp %s/lock/nodes /etc/ctdb/nodes' % config_dir], expr_form='list')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
        ...
        # print "Removing original CTDB public_addresses file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'rm -f /etc/ctdb/public_addresses'], expr_form='list')
        # print "Removed original CTDB public_addresses file on GRIDCell
        # %s."%','.join(hostname_list)

        # print "Placing the CTDB public_addresses file on GRIDCell
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'cp %s/lock/public_addresses /etc/ctdb/public_addresses' % config_dir], expr_form='list')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
...
def start_or_stop_services(client, hostname_list, action):
    ...
    try:
        ...
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'service ctdb %s' % action], expr_form='list')
        # print r1
        if r1:
            for node, ret in r1.items():
                # print ret
...
def chkconfig_services(client, hostname_list, state):
    ...
    try:
        ...
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'chkconfig ctdb %s' % state], expr_form='list')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
...
def create_admin_volume(client, admin_gridcells):
    ...
    try:
        ...
        r1 = client.cmd(admin_gridcells, 'cmd.run_all', [
                        'zfs create frzpool/normal/%s' % (admin_vol_name)], expr_form='list')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
                    raise Exception(
                        "Error creating the brick path ZFS dataset on %s" % node)
                else:
        ...
        for admin_gridcell in admin_gridcells:
            brick_list += ' %s:/frzpool/normal/%s' % (
                admin_gridcell, admin_vol_name)
        if len(admin_gridcells) > 2:
            cmd = "gluster --mode=script volume create %s repl 3 arbiter 1  %s force --xml" % (
                admin_vol_name, brick_list)
        else:
            cmd = "gluster --mode=script volume create %s repl 2  %s force --xml" % (
                admin_vol_name, brick_list)
        # print cmd
    ...
    '''
        ...
        d, err = xml_parse.run_gluster_command(
            'gluster volume start %s --xml' % admin_vol_name)
        if err:
...
def remove_admin_volume(client):
    try:
        ...
        d, err = xml_parse.run_gluster_command(
            'gluster --mode=script volume stop %s force --xml' % admin_vol_name)
        if err:
        ...
        r1 = client.cmd('roles:master', 'cmd.run_all', [
                        'zfs destroy frzpool/normal/%s' % (admin_vol_name)], expr_form='grain')
        if r1:
            for node, ret in r1.items():
                # print ret
                if ret["retcode"] != 0:
...
def mount_admin_volume(client, hostname_list):
    ...
    try:
        ...
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'mount -t glusterfs localhost:/%s %s' % (admin_vol_name, config_dir)], expr_form='list')
        # print r1
        if r1:
            for node, ret in r1.items():
                print node, ret
...
def unmount_admin_volume(client, hostname_list):
    ...
    try:
        ...
        # print "Unmounting the IntegralStor Administration volume on GRIDCells
        # %s."%','.join(hostname_list)
        r1 = client.cmd(hostname_list, 'cmd.run_all', [
                        'umount %s' % config_dir], expr_form='list')
        # print r1
        if r1:
            for node, ret in r1.items():
                # print ret
...
def add_a_gridcell_to_storage_pool(si, hostname):
    ...
    try:
        ...
        if not si:
            raise Exception(
                "Could not determine the configuration of the system!")
        if not hostname:
            raise Exception("GRIDCell not found!")
        if hostname not in si:
            raise Exception(
                "Could not determine the configuration of the GRIDCell!")
        ...
        if not ip:
            raise Exception(
                "Could not determine the IP of the specified GRIDCell")
        ...
        if not rc:
            ...
            rc1, err = revert_add_a_gridcell_to_storage_pool(
                si, client, hostname, ip)
            if err:
        ...
        return_dict, err = gluster_trusted_pools.add_a_gridcell_to_gluster_pool(
            hostname)
        print return_dict, err
        if err:
            error_list.append(err)
            rc1, err = revert_add_a_gridcell_to_storage(
                si, client, hostname, ip)
            if err:
                error_list.append(err)
            raise Exception(','.join(error_list))
    ...
    '''
        ...
        r2 = client.cmd(hostname, 'cmd.run_all', [
                        'cp %s/rc_local/rc.local /etc/rc.local' % defaults_dir])
    ...
    '''
        ...
        # On concurrent contention to acquire access token for the
        # lock file, CTDB restricts access to the lock file entirely
        # for a while; even bans a node if it contends repeatedly.
        # This backoff results in a node reload error and our script
        # will raise exception and bail out aswell.
        # This reload is not required. CTDB picks up the changes to
        # the nodes file by itself.
        '''
...
def revert_add_a_gridcell_to_storage_pool(si, client, hostname, ip):
    ...
    try:
        error_list = []
        d, error = gluster_trusted_pools.remove_a_gridcell_from_gluster_pool(
            hostname)
        if not d and error:
            error_list.append(error)
        ...
        if client:
            ...
            rc, error = copy_appropriate_rc_local(
                client, [hostname], 'remove_from_grid')
            if not rc and error:
                error_list.append(error)
...
def remove_a_gridcell_from_storage_pool(si, hostname):
    ...
    '''
        ...
        if not rc:
            error_list.append(error)
            rc1, err = revert_remove_a_gridcell_from_storage_pool(
                si, client, hostname, ip)
            if err:
                error_list.append(err)
            raise Exception(','.join(error_list))
        ...
        return_dict, err = gluster_trusted_pools.remove_a_gridcell_from_gluster_pool(
            hostname)
        # print return_dict, err
        if not return_dict:
            error_list.append(err)
        ...
        print str(e)
        return None, 'Error removing a GRIDCell from the storage pool : %s' % str(e)
    else:
        return return_dict, None
...
def revert_remove_a_gridcell_from_storage_pool(si, client, hostname, ip):
    ...
    try:
        ...
        d, error = gluster_trusted_pools.add_a_gridcell_to_gluster_pool(
            hostname)
        if error:
            error_list.append(error)
...
def add_gridcells_to_grid(request, pending_minions, admin_gridcell_list, first_time=False, print_progress=False, admin_gridcells=False, restart_salt_minions=True, establish_cron=True):
    ...
    errors = ""
    try:
        # The list of minions that got accepted but some following operations
        # failed after that.
        ...
        if pending_minions:
            ...
            accept_results, err = accept_salt_keys(
                wheel, pending_minions, print_progress)
            if err:
                raise Exception(err)
            ...
            if 'accepted_gridcells' in accept_results and accept_results['accepted_gridcells']:
                successful_gridcells.extend(
                    accept_results['accepted_gridcells'])

                # Instead, keep pinging the hosts to make sure they are all
                # contactable before continuing..
                pending_response_list = list(successful_gridcells)
                # Uncomment the following block to accomodate adding a node to
                # the GridCell through IntegralViews from any admin node
                """
        rc = client.cmd(pending_response_list,'integralstor.configure_minion',admin_gridcell_list, expr_form='list')
        time.sleep(2)
        if restart_salt_minions:
          rc = client.cmd(pending_response_list,'cmd.run',['echo service salt-minion restart | at now + 1 minute'], expr_form='list')
        print 'Minion configuration is being  updated, sleeping for 45 seconds'
        time.sleep(30)
        """

                r1 = {}
                r1 = client.cmd(pending_response_list,
                ...
                if successful_gridcells:
                    # Some succeeded so sync salt modules to those and
                    # regenerate the manifest and status.
                    if print_progress:
                        print
                    ...
                    if print_progress:
                        ...
                        print
                    # Now poll till we get a response from the module on all
                    # those gridcells
                    pending_response_list = list(successful_gridcells)
                    r1 = client.cmd(pending_response_list,
                                    'integralstor.status', expr_form='list')
                    count = 1
                    while count <= 60:
                        print 'count - ', count
                        if r1:
                            ...
                            if pending_response_list:
                                ...
                                time.sleep(1)
                                r1 = client.cmd(
                                    pending_response_list, 'integralstor.status', expr_form='list')
                            else:
                                # Phew! Done early so break out
                                print 'breaking'
                                break
          ...
          '''
                ...
                if successful_gridcells and minion_list:
                    ...
                    for gridcell_hostname in successful_gridcells:
                        # Add the IP/HN of the new gridcells to all the current
                        # minions
                        ret, err = dns.add_to_dns(
                            client, gridcell_hostname, mapping_dict[gridcell_hostname])
                        if err:
                            successful_gridcells.remove(gridcell_hostname)
                            failed_gridcells.append(gridcell_hostname)
                        # Populate the IP/HN of the all the current minions
                        # into the new gridcell
                        ret, err = dns.populate_hosts_file(
                            client, gridcell_hostname, mapping_dict)
                        if err:
                            successful_gridcells.remove(gridcell_hostname)
                            failed_gridcells.append(gridcell_hostname)
                        else:
                            ...
                            # If it is part of the first time setup, then dont
                            # audit..
                            if not first_time:
                                ret, err = audit.audit(
                                    "hardware_scan_node_added", "Added a new GRIDCell %s to the grid" % gridcell_hostname, request)
                                if err:
                                    errors += err

                # Some failed so kick off minion restart and then delete their
                # keys so that we can try again later
                if failed_gridcells:
                    if print_progress:
                ...
                if successful_gridcells:
                    ...
                    # We now have a new minion so we need to update the
                    # manifest so that the appropriate status for the new
                    # minions will also get pulled in.
                    ...
                    try:
                        ...
                        rc = client.cmd(
                            successful_gridcells, 'integralstor.configure_minion', admin_gridcell_list, expr_form='list')
                        if admin_gridcells:
                            rc = client.cmd(
                                successful_gridcells, 'integralstor.flag_as_admin_gridcell', expr_form='list')
                        if restart_salt_minions:
                            rc = client.cmd(successful_gridcells, 'cmd.run', [
                                            'echo service salt-minion restart | at now + 1 minute'], expr_form='list')
                    except Exception, e:
                        raise Exception(e)
...
def remove_a_gridcell_from_grid(gridcell_name):
    try:
        ...
        print ret, err
        ret, err = dns.unpopulate_hosts_file(
            client, gridcell_name, mapping_dict)
        print ret, err
        rc = client.cmd(gridcell_name, 'cmd.run_all', [
        ...
        with open('/proc/self/mounts', 'r') as f:
            for line in f:
                if admin_vol_name in line and config_dir in line:
                    mounted = True
                    break
        with open('%s/lock/assert_admin_vol_mount' % config_dir, 'rb') as f:
            pass
    except Exception, e:
...
def is_admin_vol_mounted_grid(client, hostname_list):
    ...
    try:
        ...
        r1 = client.cmd(hostname_list, 'mount.is_mounted',
                        [config_dir], expr_form='list')
        # print r1
        if r1:
            for node, ret in r1.items():
                return_dict[node] = ret
...
def setup_cron(client, hostname_list, admin_gridcells=False):
    try:
        jobs = [
            ...
            ['*', '*', '*', '*', '*', '/usr/bin/python /opt/integralstor/integralstor_utils/scripts/python/task_processor.py > /tmp/task_processor_output >> /tmp/task_processor_errors'],
        ...
        admin_jobs = [
            ...
            ['*/5', '*', '*', '*', '*', '/usr/bin/python /opt/integralstor/integralstor_utils/scripts/python/generate_status.py > /tmp/status_output >> /tmp/status_errors'],
            ...
            ['0', '0', '*', '*', '*', '/usr/bin/python -c "from integralstor_utils import logs; logs.auto_rotate_logs()" > /tmp/auto_rotate_logs_alerts >> /tmp/auto_rotate_errors']]
        if admin_gridcells:
            jobs.extend(admin_jobs)
        for job in jobs:
            job.insert(0, 'root')
            rc = client.cmd(hostname_list, 'cron.set_job',
                            job, expr_form='list')
            # print job
            # print rc
            for node, ret in rc.items():
                if ret not in ['new', 'updated']:
                    raise Exception(
                        'Error installing cron job %s on node %s' % (job, node))
    except Exception, e:
...
def undo_setup_cron(client, hostname_list, admin_gridcells=False):
    try:
        jobs = [
            ['/usr/bin/python /opt/integralstor/integralstor_gridcell/scripts/python/mount_admin_volume.py > /tmp/mount_admin_vol_out >> /tmp/mount_admin_vol_err'],
            ['/usr/bin/python /opt/integralstor/integralstor_utils/scripts/python/run_scheduler.py > /tmp/scheduler_output >> /tmp/scheduler_errors'],
            ['/usr/bin/python /opt/integralstor/integralstor_gridcell/scripts/python/batch_process.py > /tmp/batch_process_output >> /tmp/batch_process_errors'],
            ['/usr/bin/python /opt/integralstor/integralstor_gridcell/scripts/python/log_backup.py backup_gridcell_logs > /tmp/gridcell_log_backup_output >> /tmp/gridcell_log_backup_errors'],
            ['/usr/bin/python /opt/integralstor/integralstor_gridcell/scripts/python/config_backup.py backup_gridcell_config > /tmp/gridcell_config_backup_output >> /tmp/gridcell_config_backup_errors']]

        admin_jobs = [
            ['/usr/bin/python /opt/integralstor/integralstor_utils/scripts/python/generate_status.py > /tmp/status_output >> /tmp/status_errors'],
            ['/usr/bin/python /opt/integralstor/integralstor_gridcell/scripts/python/poll_for_alerts.py > /tmp/gridcell_poll_for_alerts_output >> /tmp/gridcell_poll_for_alerts_errors'],
            ['/usr/bin/python -c "from integralstor_utils import logs; logs.auto_rotate_logs()" > /tmp/auto_rotate_logs_alerts >> /tmp/auto_rotate_errors']]
        if admin_gridcells:
            jobs.extend(admin_jobs)
        for job in jobs:
            job.insert(0, 'root')
            rc = client.cmd(hostname_list, 'cron.rm_job',
                            job, expr_form='list')
            print job
            print rc
            for node, ret in rc.items():
                if ret not in ['removed', 'absent']:
                    raise Exception(
                        'Error removing cron job %s on node %s' % (job, node))
    except Exception, e:
        return False, 'Error setting up cron : %s' % str(e)
    else:
        return True, None
...
def main():
    #_regenerate_manifest_and_status()
