...
def remove_a_gridcell_from_storage_pool(si, hostname):
  ...
  try :
    ...
    return_dict, err  = gluster_trusted_pools.remove_a_gridcell_from_gluster_pool(hostname)
    #print return_dict, err
    if not return_dict:
      error_list.append(err)
      rc1, err = revert_remove_a_gridcell_from_storage_pool(si, client, hostname, ip)
  ...
  try:
    ...
    if pending_minions:
      ...
      if 'accepted_gridcells' in accept_results and accept_results['accepted_gridcells']:
        ...
        # Uncomment the following block to accomodate adding a node to the GridCell through IntegralViews from any admin node
        """
