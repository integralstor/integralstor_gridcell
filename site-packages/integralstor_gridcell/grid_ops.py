...
def create_admin_volume(client, admin_gridcells):
  ...
  try :
    ...
    cmd = "gluster --mode=script volume create %s repl 3  %s force --xml"%(admin_vol_name, brick_list)
    #print cmd
...
def check_if_admin_vol_mounted(client, hostname_list):
...
def add_gridcells_to_grid(request_meta,pending_minions, admin_gridcell_list, first_time = False, print_progress = False, admin_gridcells = False):
  ...
  try:
    ...
    if pending_minions:
      ...
      ret, err = accept_salt_keys(wheel, pending_minions, print_progress)
      if err:
        raise Exception(err)
      if print_progress and ret and 'accepted_gridcells' in ret:
        print 'Accepted GRIDCells %s.'%','.join(ret['accepted_gridcells'])
      ...
      if print_progress:
        print "\nPausing 30 seconds to salt accept\n"
      time.sleep(30)

      if 'accepted_gridcells' in ret and ret['accepted_gridcells']:
        for gridcell_hostname in ret['accepted_gridcells']:
          #Get the bond IP for each of the minions.
          if print_progress:
            print 
            print 'Getting bond0 IP for GRIDCell %s'%gridcell_hostname
          ip, err = get_minion_ip(client, gridcell_hostname)
          if print_progress and err:
            print "Error retrieving the IP from GRIDCell %s : %s"%(gridcell_hostname, err)
          if not ip:
            if print_progress:
              print  "Error retrieving the IP from GRIDCell %s : %s"%(gridcell_hostname, err)
            accepted_failed_minions.append(gridcell_hostname)
            errors += "Error retrieving the IP from GRIDCell %s : %s"%(gridcell_hostname, err)
            #Cannot add to DNS so remove from salt as well
            failed_gridcells.append(gridcell_hostname)
            continue
          else:
            if print_progress:
              print "Found Bond IP : %s for GRIDCell %s"%(ip, gridcell_hostname)
  
          #Accepted into salt and found the IP so now add to DNS.
          if print_progress:
            print 'Adding GRIDCell %s to DNS'%gridcell_hostname
          rc, err = dns.add_to_dns(client, gridcell_hostname, ip)
          if err and print_progress:
            print 'Error adding GRIDCell %s to DNS : %s'%(gridcell_hostname, err)
          if not rc :
            accepted_failed_minions.append(gridcell_hostname)
            errors += err
            failed_gridcells.append(gridcell_hostname)
            ...
            if print_progress:
              print 'Adding GRIDCell %s to DNS.. Done.'%gridcell_hostname
          ...
          if not first_time:
            ...
            if err:
              errors += err
          successful_gridcells.append(gridcell_hostname)
        ...
        if accepted_failed_minions:
          ...
          if err:
            errors += err
          for gridcell_hostname in accepted_failed_minions:
