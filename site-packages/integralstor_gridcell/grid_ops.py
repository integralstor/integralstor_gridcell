...
def add_to_dns(client, m, ip):
  try:
    ...
    if m not in ['fractalio-pri.fractalio.lan', 'fractalio-sec.fractalio.lan', 'fractalio-pri', 'fractalio-sec']:
...
def mount_admin_volume(client, hostname_list):
  try :
    ...
    r1 = client.cmd(hostname_list, 'cmd.run_all', ['mount -t glusterfs fractalio-pri.fractalio.lan:/%s %s'%(admin_vol_name, config_dir)], expr_form='list')
    #print r1
    if r1:
      for node, ret in r1.items():
        #print ret
...
def add_nodes_to_grid(remote_addr,pending_minions, first_time = False, accessing_from = 'primary', print_progress = False):
  ...
  try:
    ...
    if pending_minions:
      ...
      if print_progress:
        print "Pausing 20 seconds to salt accept"
      time.sleep(20)
      for m in pending_minions:
